...
export const getPaymentTypeByPaymentId = async paymentId => {
    try {
        const response = await apiClient.get(`/payment_types?payment_id=${paymentId}`);
        return response.data;
    } catch (error) {
        console.error(`Error fetching payment type for payment ID ${paymentId}:`, error);
        throw error;
    }
};
