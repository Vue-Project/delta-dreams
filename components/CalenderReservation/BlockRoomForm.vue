        ...
        <form id="formBlockRoom" class="p-2" @submit.prevent="submitFormBlockRoom" ref="emptyForm">
            <div class="row">
                ...
                <div class="col-12">
                    ...
                    <div class="text-danger small" v-if="$v.formBlock.roomType.$error">Room is required</div>
                </div>
                <div class="col-12">
                    ...
                    <div class="text-danger small" v-if="$v.formBlock.reason.$error">Please select a reason</div>
                </div>
            </div>
        ...
        methods: {
            ...
            splitResourceId() {
                if (this.selectedResourceId) {
                    // For format like "18/1 - ID: 33-73 - Project: 1"
                    if (this.selectedResourceId.includes('ID:')) {
                        // Extract the ID part (33-73)
                        const idMatch = this.selectedResourceId.match(/ID:\s*(\d+-\d+)/);
                        if (idMatch && idMatch[1]) {
                            // Get the room type from the first part
                            this.formBlock.roomType = this.selectedResourceId.split('-')[0].trim();

                            // Extract the last number (73) from the ID part (33-73)
                            const idParts = idMatch[1].split('-');
                            this.formBlock.room = idParts[idParts.length - 1];
                        }
                    } else {
