    ...
    <form ref="guestForm" id="formGustInfo" @submit.prevent="submitFormUpdateGuest">
        <fieldset :disabled="!isClientEditable">
            ...
            <DropzoneComponent :id="'my-dropzone'" />
                ...
                <!-- ...existing code... -->
                <div class="my-3">
                    <label class="form-label">Related Clients</label>
                    <div v-for="(client, idx) in relatedClients" :key="idx" class="row align-items-end mb-2">
                        <div class="col-md-3">
                            <input class="form-control" type="text" v-model="client.name" placeholder="Name" />
                        </div>
                        <div class="col-md-3">
                            <input class="form-control" type="text" v-model="client.idNumber" placeholder="ID Number" />
                        </div>
                        <div class="col-md-3 position-relative">
                            <input class="form-control" type="file" accept="image/*" @change="handleRelatedClientImage($event, idx)" style="padding-right: 80px" />
                            <!-- <span v-if="relatedClientsPreview[idx]?.name" style="top: 8px; right: 10px; background: #fff; padding: 0 4px; font-size: 12px">
                                {{ relatedClientsPreview[idx].name }}
                            </span> -->
                        </div>
                        <div class="col-md-2">
                            <input class="form-control" type="text" v-model="client.relation" placeholder="Relation" />
                        </div>
                        <div class="col-md-1">
                            <button type="button" class="btn btn-danger" @click="removeRelatedClient(idx)" v-if="relatedClients.length > 1">
                                <i class="fa fa-trash"></i>
                            </button>
                        </div>
                    </div>
                    <button type="button" class="btn btn-primary btn-sm mt-2" @click="addRelatedClient">
    ...
    export default {
        ...
        data() {
            return {
                ...
                imgs: [],
                isSubmitting: false,
                relatedClients: [{ name: '', idNumber: '', image: null, relation: '' }],
                relatedClientsPreview: [],
            };
        },
        validations: {
            formGuest: {
        ...
        methods: {
            ...
            async submitFormUpdateGuest() {
                ...
                try {
                    ...
                    // Append related clients
                    this.relatedClients.forEach((relatedClient, idx) => {
            ...
            fillFormGuest(reservationData) {
                ...
                if (client.userGalleries?.data && Array.isArray(client.userGalleries.data)) {
                    ...
                    this.IdImagesPreview = client.userGalleries.data.map((item, index) => {
                        ...
                        const imageUrl = item.image.startsWith('http') ? item.image : `${baseUrl}${item.image.startsWith('/') ? '' : '/'}${item.image}`;
                ...
                if (client.national_id_image) {
                    ...
                    const nationalIdUrl = client.national_id_image.startsWith('http') ? client.national_id_image : `${baseUrl}${client.national_id_image.startsWith('/') ? '' : '/'}${client.national_id_image}`;
            ...
            addRelatedClient() {
                this.relatedClients.push({ name: '', idNumber: '', image: null, relation: '' });
                this.relatedClientsPreview.push(null);
            },
            removeRelatedClient(index) {
                this.relatedClients.splice(index, 1);
                if (this.relatedClientsPreview[index] && this.relatedClientsPreview[index].url) {
                    URL.revokeObjectURL(this.relatedClientsPreview[index].url);
                }
                this.relatedClientsPreview.splice(index, 1);
            },
            handleRelatedClientImage(event, index) {
                const file = event.target.files[0];
                if (file && file.type.startsWith('image/')) {
                    this.relatedClients[index].image = file;
                    const fileURL = URL.createObjectURL(file);
                    // Clean up previous preview if exists
                    if (this.relatedClientsPreview[index] && this.relatedClientsPreview[index].url) {
                        URL.revokeObjectURL(this.relatedClientsPreview[index].url);
                    }
                    this.relatedClientsPreview[index] = { url: fileURL, name: file.name };
                }
                // Set the input value to the file name for display
